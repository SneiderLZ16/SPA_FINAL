...
function showRegister() {
  ...
  document.getElementById('registerForm').onsubmit = async e => {
    e.preventDefault();
    const data = Object.fromEntries(new FormData(e.target));
    if (await isUserOrEmailTaken(data.username, data.email)) {
...
// ------------------ UNIRSE A EVENTO ------------------
/**
 * Permite al usuario actual unirse a un evento por ID.
 * Verifica si ya está inscrito o si el evento está lleno antes de inscribirlo.
 */
...
//------------------- EDITAR EVENTO ------------------
    async function showEditEvent(id) {
      ...
      main.innerHTML = `
        <form id="editEventForm">
          <h3>Editar Evento</h3>
          <input name="name" value="${event.name}" placeholder="Nombre del evento" required />
          <textarea name="description" placeholder="Descripción" required>${event.description}</textarea>
          <input name="fecha" type="date" value="${event.fecha}" required />
          <input name="capacidad" type="number" min="1" value="${event.capacidad}" placeholder="Capacidad máxima" required />
          <button type="submit">Guardar cambios</button>
          <button type="button" onclick="showDashboard()">Cancelar</button>
        </form>
      `;
    ...
    // Modifica el botón de edición en el listado de eventos para admin
        ...
        container.innerHTML = events.map(ev => {
          ...
          return `
            <div class="event-card">
              ...
              ${user.role === 'admin' ? `
                <button onclick="deleteEvent(${ev.id})">Eliminar</button>
                <button onclick="showEditEvent(${ev.id})">Editar</button>
              ` : ''}
              ${user.role === 'visitor' && !yaInscrito && disponible > 0 ? `
                <button onclick="joinEvent(${ev.id})">Unirme</button>
              ` : user.role === 'visitor' && yaInscrito ? `
